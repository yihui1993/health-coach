...
async function loadHome() {
  ...
  if (!stats.initialized) {
    document.getElementById('stats-grid').innerHTML = '<p style="color:var(--muted);grid-column:1/-1">No health data yet. Start logging via Telegram!</p>';
    return;
  }
...
async function loadWorkouts() {
  ...
  if (!data.length) {
    container.innerHTML = '<p style="color:var(--muted)">No workouts logged yet. Send a gym screenshot via Telegram!</p>';
    return;
  }
...
async function loadNutrition() {
  ...
  if (!meals.length) {
    list.innerHTML = '<p style="color:var(--muted)">No meals logged yet. Send a food photo via Telegram!</p>';
    return;
  }
